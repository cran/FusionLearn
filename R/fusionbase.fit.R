fusionbase.fit<-function(x,y,lambda,N,p,m,beta=0.1,thresh=0.05,maxiter=30,
                        methods="scad", Complete=TRUE, depen ="IND", a=1){
  ##Sample size
  if (length(N)==1) {
    N <- rep(N, m)
  } 
  ...
  for ( k in 1:n){
    ##Fusion method
    result = fusionbase(x,y,lambda[k],N,p,m,beta,thresh,maxiter,methods,
                       Complete)
    result = result$beta
    if ( Complete == FALSE){
      result[is.na(result)] <- 0
      index <- rowSums(abs(result))!=0
    }
    index <- result[,1]!=0
    p1 <- sum (index)
    if (p1==0){
      output[k,] = NA
      commen[k]= "p == 0, penalty value is too large"
      next
    }
    if (p1 >= min(N)){
      output[k,] = NA
      commen[k] = "p >= n, glm cannot converge"
      next
    }
    ...
    if(det(informat)==0){
      output[k,] = NA
      commen[k] = "information matrix computationally singular"
      next
    }
    if("IND" %in% depen){
      for(s in 1:m){
