fusionbinary<-function(x,y,lambda,N,p,m,beta=0.1,thresh=0.1,maxiter=100, 
  ...
  while ((diff>thresh)&&(niter<maxiter)){
    for (s in 1:m){
      ...
      if ("probit" %in% link){
        vu=2/pi
        linearp<-x[,s,]%*%beta[,s] #normal? probit?
    ...
    for (j in 1:p){
      ...
      if ("lass" %in% methods ){
        beta[j,]<- .lass(norm*vu,lambda)*(z[j,])/(norm*vu)
      }
      if ("scad" %in% methods){
        beta[j,]<- .scad(norm*vu,lambda)*(z[j,])/(norm*vu)
      }
      for (s in 1:m){
